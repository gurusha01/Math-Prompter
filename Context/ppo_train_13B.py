...
Q_pnc, A_pnc, COT_pnc, T_pnc = create_dataset_MATH("./alpaca-lora/MATH/test/counting_and_probability", 1)
Q_NT, A_NT, COT_NT, T_NT = create_dataset_MATH("./alpaca-lora/MATH/test/number_theory", 1)
Q_ialg, A_ialg, COT_ialg, T_ialg = create_dataset_MATH("./alpaca-lora/MATH/test/intermediate_algebra", 1)
Q_alg, A_alg, COT_alg, T_alg = create_dataset_MATH("./alpaca-lora/MATH/test/algebra", 1)
Q_AQuA, A_AQuA, COT_AQuA, T_AQuA = create_dataset_AQuA("./alpaca-lora/AQuA/test.json", 1)
# PnC 250, NT 250, IAlg 250, Alg 250, SQ 500, AQuA 254

Q =  Q_pnc[:250] + Q_AQuA[:254] + Q_NT[:250] + Q_ialg[:250] + Q_alg[:250] + Q_stratergy[:500]
A =  A_pnc[:250] + A_AQuA[:254] + A_NT[:250] + A_ialg[:250] + A_alg[:250] + A_stratergy[:500]
T =  T_pnc[:250] + T_AQuA[:254] + T_NT[:250] + T_ialg[:250] + T_alg[:250] + T_SQ[:500]
COT = COT_pnc[:250] + COT_AQuA[:254] + COT_NT[:250] + COT_ialg[:250] + COT_alg[:250] + COT_stratergy[:500]
...
for _ in range(num_epochs):
    ...
    for batch in tqdm(train_dataloader):
        ...
        if(batch_number%10 == 0):
            ppo_trainer.model.save_pretrained("./openAI_trials/Context")
